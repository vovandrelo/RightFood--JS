        ...
        // Если ошибки не произошло, то:
            ...
            if (blocks) {
                ...
                blocks.forEach(file => {
                    ...
                    // Если текущее название - название pug-файла, то:
...
const createImportedPath = (path, fileName) => {
    return `include ../../${path.substring(path.indexOf('blocks'))}${fileName}\n`;
};

// Функция находит пути до необходимых pug-файлов и подключает их в blocks.pug
export const includePug = cb => {
    // Отчищаем текущий файл с импортами блоков:
    fs.writeFileSync(path.pugInclude, "");

    // Выполняем основную задачу функции:
    searchPugFiles(path.blocks, path.pugInclude);
