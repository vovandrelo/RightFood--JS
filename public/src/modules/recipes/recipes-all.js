...
//<==================================================================================================================>\\
//<============================== РЕАЛИЗАЦИЯ ЛОГИКИ РАБОТЫ СТРАНИЦЫ СО ВСЕМИ РЕЦЕПТАМИ ==============================>\\
//<==================================================================================================================>\\
...
const recipeAll = () => {
    ...
    service.getData(`/recipes/all/temp/${pageConfig.temp}/kcal/${pageConfig.kcal}/page/${pageConfig.numPage}`)
    .then(res => {
        res.prodCard.forEach(dataCard => {
            ...
            if (admin === "true") {
                ...
                newCard = new Card(dataCard, false, true);
            }
            ...
            .then(res => {
                if (res.prodCard.length === 0) {
                    ...
                    res.prodCard.forEach(dataCard => {
                        let newCard = null;
                        if (admin === "true") {
                            newCard = new Card(dataCard, true, true);
                        } else {
                            newCard = new Card(dataCard, false, true);
                        }
                        cardsItems.push(newCard);
    ...
    const bindModal = newCard => {
        newCard.elem.addEventListener("click", event => {
            ...
            } else if (event.target.classList.contains('icon-add')) {
                service.getData(`/recipes/user/addFavourite/${newCard.recipeId}`, JWT)
                .then(res => {
                    console.log("Рецепт успешно удалён в избранное!");
                })
                .catch(err => {
                    console.log(err);
                });
